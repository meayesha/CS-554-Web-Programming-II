...
async function getCharacters(pageNum){
    const { data } = await axios.get('https://gateway.marvel.com:443/v1/public/characters'+ '?ts=' + ts + '&apikey=' + publickey + '&hash=' + hash+'&limit=20&offset=' + (pageNum * 20).toString())
  ...
  async function getCharactersById(id){
    console.log("Inside the get charatcers function");
    ...
    const { data } = await axios.get(`https://gateway.marvel.com:443/v1/public/characters/${id}` + '?ts=' + ts + '&apikey=' + publickey + '&hash=' + hash);
    //const parsedData = JSON.parse(data) // parse the data from JSON into a normal JS Object
    console.log(data);
   if(data == null) throw `No character found with id ${id}.`
   return data ;// this will be the array of people objects
  }

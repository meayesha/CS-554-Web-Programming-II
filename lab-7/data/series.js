...
const currPage = 1;
async function getSeries(pageNum){
    const { data } = await axios.get('https://gateway.marvel.com:443/v1/public/series'+ '?ts=' + ts + '&apikey=' + publickey + '&hash=' + hash+'&limit=20&offset=' + (pageNum * 20).toString())
  ...
  async function getSeriesById(id){
    ...
    const { data } = await axios.get(`https://gateway.marvel.com:443/v1/public/series/${id}` + '?ts=' + ts + '&apikey=' + publickey + '&hash=' + hash);
   // const parsedData = JSON.parse(data) // parse the data from JSON into a normal JS Object
   if(data == null) throw `No series found with id ${id}.`
   return data ;// this will be the array of people objects
  }
